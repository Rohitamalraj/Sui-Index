...
const rows = [
  { feature: "MULTI-ASSET INDEXES",    si: "[✓]", poly: "[—]", kalshi: "[✗]", drift: "[—]" },
  { feature: "ON-CHAIN ESCROW",        si: "[✓]", poly: "[✓]", kalshi: "[✗]", drift: "[✓]" },
  { feature: "ORACLE-BASED SETTLEMENT",si: "[✓]", poly: "[—]", kalshi: "[—]", drift: "[✓]" },
  { feature: "IMMUTABLE RECORDS",      si: "[✓]", poly: "[—]", kalshi: "[✗]", drift: "[—]" },
  { feature: "SOCIAL INDEX CARDS",     si: "[✓]", poly: "[✗]", kalshi: "[✗]", drift: "[✗]" },
  { feature: "NO KYC REQUIRED",        si: "[✓]", poly: "[✓]", kalshi: "[✗]", drift: "[✓]" },
];
...
export default function Comparison() {
  return (
    <section id="comparison" className="flex flex-col w-full bg-[#050505] py-16 px-6 md:py-[100px] md:px-[120px] gap-12 md:gap-[64px]">
      <SectionHeader
        label="[05] // VS. THE REST"
        title={"WHY SUI-INDEX\nWINS."}
        subtitle="HEAD-TO-HEAD AGAINST EXISTING PREDICTION PLATFORMS. NO SPIN. JUST FACTS."
      />
      ...
      <div className="hidden md:flex flex-col w-full border border-[#2D2D2D]">
        <div className="flex w-full h-[56px] bg-[#111111] border-b-2 border-b-[#FFD600]">
          ...
          <div className="flex items-center flex-1 px-[32px] bg-[#1A1A1A] border-r border-r-[#2D2D2D]">
            <span className="font-grotesk text-[11px] font-bold text-[#FFD600] tracking-[2px]">SUI-INDEX</span>
          </div>
          {["POLYMARKET", "KALSHI", "DRIFT"].map((tool, i) => (
        ...
        {rows.map((row, i) => (
          <div key={row.feature} className={`flex w-full h-[56px] ${i < rows.length - 1 ? "border-b border-b-[#1D1D1D]" : ""}`}>
            ...
            <div className="flex items-center flex-1 px-[32px] bg-[#0D0D0D] border-r border-r-[#2D2D2D]">
              <span className="font-ibm-mono tracking-[1px] text-[#FFD600] font-bold text-[14px]">{row.si}</span>
            </div>
            {[row.poly, row.kalshi, row.drift].map((val, j) => (
      ...
      <div className="flex flex-col md:hidden w-full gap-[2px]">
        <div className="grid grid-cols-5 bg-[#111111] border border-[#FFD600] border-b-2">
          ...
          <div className="px-2 py-3 bg-[#1A1A1A]">
            <span className="font-grotesk text-[9px] font-bold text-[#FFD600] tracking-[1px]">SI</span>
          </div>
          <div className="px-2 py-3">
            <span className="font-grotesk text-[9px] font-bold text-[#555555] tracking-[1px]">PLY</span>
          </div>
          <div className="px-2 py-3">
            <span className="font-grotesk text-[9px] font-bold text-[#555555] tracking-[1px]">KSH</span>
          </div>
        </div>
        {rows.map((row, i) => (
          <div key={row.feature} className={`grid grid-cols-5 border border-[#1D1D1D] ${i % 2 === 0 ? "bg-[#0A0A0A]" : "bg-[#0D0D0D]"}`}>
            ...
            <div className="flex items-center px-2 py-4 bg-[#0D0D0D]">
              <span className="font-ibm-mono text-[12px] text-[#FFD600] font-bold">{row.si}</span>
            </div>
            <div className="flex items-center px-2 py-4">
              <span className={`font-ibm-mono text-[11px] ${cellColor(row.poly)}`}>{row.poly}</span>
            </div>
            <div className="flex items-center px-2 py-4">
              <span className={`font-ibm-mono text-[11px] ${cellColor(row.kalshi)}`}>{row.kalshi}</span>
            </div>
          </div>
        ))}
      </div>
